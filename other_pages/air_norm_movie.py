...
all_test_count_maps = None
with cols[0]:  # PROCESS TEST DATA
    if test_data:
        ...
        cc_data_dict, params_info, params = get_data_info(test_files)
        max_frames = cc_data_dict["Capture views"]
        pixel_rows = cc_data_dict["Pixel rows"]
        pixel_cols = cc_data_dict["Pixel columns"]

        frame_selection = st.sidebar.slider("Frame", 0, max_frames-1, 0)
        average_over_frames = st.sidebar.checkbox("Average over frames", value=False)
        if average_over_frames:
            frame_selection = None

        all_test_count_maps = np.zeros((7, max_frames, pixel_rows*8, pixel_cols*2))

        with st.expander("Extracted files and folders", expanded=False):
            for file in test_files:
                st.write(file.name)

        for i, bin_id in enumerate(np.arange(0, 7)):
            for frame in range(max_frames):
                ...
                all_test_count_maps[bin_id, frame, :, :] = test_count_map
            

# with cols[0]:  # PLOT TEST DATA
#     if all_test_count_maps:
#         for i, bin_id in enumerate(bin_selection):
#             test_count_map_plot = all_test_count_maps[bin_id, frame_selection, :, :]
            
#             color_min, color_max = np.percentile(
#                 test_count_map_plot, [color_pctl_0, color_pctl_1]
#             )
...
#                 test_count_map_plot, color_range, colormap, invert_color
#             )
...
all_air_norm_count_maps = {}
with cols[1]:  # PROCESS AIR NORM DATA
    if air_norm_data:
        with st.spinner("**PROCESSING FILES...**"):
        ...
        for i, bin_id in enumerate(np.arange(0, 7)):
            ...
            all_air_norm_count_maps[bin_id] = air_norm_count_map

with cols[1]:  # PLOT AIR NORM DATA
    if all_air_norm_count_maps:
        ...
        air_norm_count_map_plot = all_air_norm_count_maps[6]

        heatmap_fig = cached_create_heatmap(
            air_norm_count_map_plot, color_range, colormap, invert_color
        )
        heatmap_fig.update_layout(title=f"{BIN_LABELS[bin_id]}")
        heatmap_fig.update_layout(autosize=False, width=400, height=fig_height)
...
with cols[2]:  # PROCESS NORMALIZED DATA
    if test_data and air_norm_data:
        
        all_normalized_count_maps = {}
        ...
        with st.spinner("**PROCESSING FILES...**"):
            for i, bin_id in enumerate(np.arange(0,7)):
                ...
                normalized_count_map = np.divide(all_test_count_maps[bin_id], 
                                                all_air_norm_count_maps[bin_id])

                # replace nans with 0
                normalized_count_map = np.nan_to_num(normalized_count_map)

                all_normalized_count_maps[bin_id] = normalized_count_map

with cols[2]:  # PLOT NORMALIZED DATA
    if all_normalized_count_maps:
        for i, bin_id in enumerate(bin_selection):
            normalized_count_map = all_normalized_count_maps[bin_id]
            color_min, color_max = np.percentile(
